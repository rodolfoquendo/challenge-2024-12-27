...
LABEL org.afluenta.php.authors="roquendo@afluenta.com"
LABEL org.afluenta.php.version="8.2"
LABEL org.afluenta.php.os="alpine"
LABEL org.afluenta.php.nginx="0"
LABEL org.afluenta.php.cron="0"
LABEL org.afluenta.php.xdebug="0"
LABEL org.afluenta.php.last_updated_at="2023-11-08 13:00:00"
EXPOSE 9000
WORKDIR /afluenta-platform
...
RUN apk add openssl
# apk add php8-pecl-imagick 1.9s
...
RUN apk add nginx
COPY images/scripts/php-nginx.sh /scripts/php-nginx.sh
EXPOSE 80 443
RUN chmod 777 /scripts/php-nginx.sh
RUN mkdir -p \
    /var/log/nginx
RUN touch \
    /var/log/nginx/access.log \
    /var/log/nginx/error.log
ENTRYPOINT /scripts/php-nginx.sh 

# If cron is needed
# FROM 599517226567.dkr.ecr.us-east-1.amazonaws.com/php8-nginx-cron
COPY configs/nginx.conf /etc/nginx/http.d/default.conf
COPY src /afluenta-platform/

RUN composer install --no-dev
RUN rm -rf /afluenta-platform/.env
RUN chmod -R 777 /afluenta-platform/storage /scripts/php-nginx.sh
# If cron is needed
# RUN chmod -R 777 /afluenta-platform/storage /scripts/php-nginx-cron.sh
