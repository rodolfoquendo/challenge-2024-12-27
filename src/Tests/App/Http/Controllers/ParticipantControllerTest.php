...
class ParticipantControllerTest extends \Tests\TestCase
{
     ...
     */
    public function test__get__no_auth(): void
    {
        $this->createApplication();
        $response = $this->getJson($this->APIRoute_v1('tournament/participants'));
        $response->assertUnauthorized();
        $response = $this->getJson($this->APIRoute_v1('tournament/participants?name=Rodolfo+Oquendo'));
        $response = $response->json();
        $response->assertTrue(isset($response['name']));
        $response->assertTrue($response['name'] == 'Rodolfo Oquendo');
